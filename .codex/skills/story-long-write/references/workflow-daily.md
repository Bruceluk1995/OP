...
- 项目已有 `正文/` 和 `追踪/` 目录
- 用户说"日更""续写""继续写"
- 目标：每次会话写 2-3 章；短季/油管朗读/推文连载/完整正文默认每章或每集 14,500-16,500 字符，普通长篇按项目细纲目标执行
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - 写正文 → **字数验证（优先 Python 字符统计，`wc -m` 仅作 Unix 备选；短季/油管朗读/推文连载/完整正文默认 14,500-16,500 字符，低于 14,500 且无用户短稿授权不得完成；普通长篇按细纲目标执行，< 目标则对照情节点字数预算定位欠账密点、一次性重写到配额，不挤牙膏反复回炉；> 章目标×1.1 则压过场/合并疏点收敛；90% 只能作为警戒线，不能作为完成线）** → 检查钩子/爽点 → **正文元信息扫描** → 禁用词扫描
