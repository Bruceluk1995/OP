...
- 项目已有 `正文/` 和 `追踪/` 目录
- 用户说"日更""续写""继续写"
- 目标：每次会话写 2-3 章；短季/油管朗读/推文连载/完整正文无明确规格时每章或每集约 12,000 字符预算，普通长篇按项目细纲目标执行
...
- 写正文 → **长度与结构复核**（Python 统计并报告实际字符数；短季/朗读/推文无明确规格时约 12,000 字符预算。低于预算只在缺少新选择、代价、关系变化或后果时返工；高于预算先删重复解释、重复证明和无状态变化流程）→ 检查钩子/兑现 → 正文元信息扫描 → 禁用词扫描
