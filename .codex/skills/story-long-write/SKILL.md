...
Required global read: `../story/references/audience-comprehension-floor.md`. Apply its plain-core and five-line gates before premise design, outlining, drafting, and revision.
...
长篇先建立持续欲望，再用人物选择制造不可逆变化；验证过的模式只提供功能，不提供可换名复刻的整条剧情。

1. **先定读者等待**：本卷/本章让读者等什么结果，主角又想得到什么；情绪必须附着在具体等待上。
2. **故事核先于设定**：普通困境 -> 主角独特选择 -> 结果制造新问题。靠运气、机构或设定说明自行推进的核心退回重做。
3. **对标只拆功能**：分别提取开头压力、人物矛盾、中段换挡、高潮依赖和结尾余味，禁止沿用一本作品的完整顺序。
4. **每章有状态差**：记录进入状态、人物选择、退出状态和下一问题；连续两章只解释、赶路、训练或重复胜利时合并/重构。
5. **中段必须换机制**：敌人学习、规则改变、代价上升、关系破裂、身份暴露或成功反噬，不能只放大第一次爽点。
6. **角色声音来自选择**：用注意点、判断、回避和代价区分人物，不用统一口癖或标签。
7. **盲读后再看指标**：先标出最早不想继续的位置并退回故事核/结构/场景，再做字数、格式和去 AI 味。
8. **只加载必需信息**：每章只读会影响当前选择的角色状态、伏笔、设定和上一章尾部。短季仍只规划首季 6 集，完整一集无指定时目标约 **12,000 字符**，不设凑字硬门槛。
...
短季长集默认以约 12,000 字符为预算。细纲先规划足够的选择、代价、关系变化和新问题，不锁死情节点数量。每个长段/场景都要改变信息、关系、身份、风险、资源、目标、敌方策略或下一选择；核心已完整时不为跨过最低字数继续扩写。
...
- 字数目标：{X} 字（短季/朗读/推文/完整正文无明确规格时约 12,000；用户目标优先）
- 目标情绪：{本章交付什么情绪}
...
#### 情节细化
- 情节点序列：每点写清“谁想要什么 -> 做了什么选择 -> 状态如何改变 -> 留下什么问题”。密疏由冲突与人物注意力决定，不给每点固定字数，不以点数反推章节长度；只有平台硬规格才做总预算。
...
8. **长度与结构复核**：用 Python 统计并报告实际字符数。低于预算时先检查是否缺少新的选择、代价、关系变化或成功后果；没有结构欠账就不补。高于预算时删重复解释、重复证明和无状态变化的流程段。长度结果不能替代盲读编辑结论。
...
| 推进与回报 | 高压章用冲突与结果推进，低压章用关系、选择、发现或准备改变下一步；每章都要让读者继续等待，但不按固定字数塞爽点（见 references/outline-structure-theory.md「章节定位与张弛」） |
