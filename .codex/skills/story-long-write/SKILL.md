...
description: "通用长篇网文与短季分集小说写作。从开书、设定、卷纲/细纲到逐章日更、续写、回炉和追踪文件维护。用于写长篇、连载、写大纲、日更、继续写、修改第X章；明确的日本男频异世界或女性向幻想恋爱优先使用对应 jp-* 专门 skill，单篇短故事用 story-short-write。"
...
- “自定义 / 其他 / 提供素材”也必须编号；选中后下一轮只索取一个必要内容。是非确认统一写成 `1. 是` / `2. 否`，并要求只回复数字。
...
5. **先定项目尺度**。用户要短季、分集、每集、油管朗读连载或 6 集时，使用短季小说模式：仍按小说章节文件写作，但固定只规划首季 6 章/集，不主动推荐更长季或百章长篇。完整正文、油管朗读、推文连载、短季每集正文默认目标为 **14,500-16,500 字符**；用户明确指定其他长度时按用户目标执行。
...
| **短季开书** | "短季"/"分集"/"每集"/"6集"/"油管朗读连载" | 完整 Phase 1→2→3→4→5，但 Phase 3 只做首季 6 章/集 |
...
缺失时显示 `1. 有选题决策文件`、`2. 没有，进入常规选题`，要求只回复数字。选 1 后下一轮只索取文件路径；选 2 进入下面的常规数字菜单。

如果用户已有方向 → 直接进入 Phase 2。

如果用户没有方向，依次进行三个数字门：

1. 情绪目标：`1 爽感释放`、`2 悬疑紧张`、`3 治愈温暖`、`4 压迫后逆袭`、`5 现实共鸣`、`6 自定义`。
2. 对标状态：`1 已有对标书`、`2 没有对标书`、`3 由系统推荐`。选 1 后下一轮只索取书名或路径。
3. 创作优势：`1 脑洞`、`2 文笔`、`3 节奏`、`4 生活经验`、`5 暂不确定`；允许用 `+` 多选。

每一门都要求用户只回复数字或数字组合；选择自定义后，下一轮只索取一个必要内容。
...
- 传统长篇：可按多卷、滚动细纲推进。
- 短季分集小说：固定首季 6 章/集。仍使用 `大纲/细纲_第XXX章.md`、`正文/第XXX章_*.md`、`追踪/`，不改成剧本格式。
- 短季模式下，`第一卷` 等价于 `首季`。只规划首季完整弧线和每集细纲；不要预设几十章、几百章后续。需要第二季时再开新卷。
...
短季分集小说默认一次建完 6 个细纲；如果用户只想先试水，可先建前 3 集，但必须同时给出首季 6 集鸟瞰。不要主动规划 6 集以外的首季长度。

短季长集功能密度规则：如果每集目标约 1.5 万字，细纲必须先规划足够有效内容，不允许靠景物、形容词、空泛情绪、氛围段落凑字。不要锁死固定情节点数量，情节点数量随本集节奏和情绪目标决定。每个长段/场景都要有明确功能：推进信息、关系、身份/地位、风险、证据、资源、目标、敌方压力、下一步选择，或给前后爽点提供必要蓄势、沉淀、转场、回收。能整段删除且不损失任何剧情、关系、证据、情绪压力、后续期待的内容，才算水。短季/朗读/推文/完整正文落盘后必须用 Python 统计字符数；低于 14,500 且用户没有明确批准短稿时，不得交付完成，必须补足有功能的情节点。
...
- 字数目标：{X} 字（短季/朗读/推文/完整正文默认 14,500-16,500；用户明确指定其他长度时按用户目标）
- 目标情绪：{本章交付什么情绪}
- 章节定位：{高压/推进/修炼试错/关系回收/低压生活/信息整理；可留空，留空按推进章。见 references/outline-structure-theory.md「章节定位与张弛」}
- 长集功能密度：{若目标约1.5万字，说明本集哪些段落承担推进/蓄势/沉淀/转场/回收功能；标出需要删除或压缩的无功能水段}
...
8. **字数验证**（写作完成后的第一件事）：优先用跨平台 Python 字符统计本章实际字数 `for PYBIN in python3 python py; do "$PYBIN" -c "" 2>/dev/null && break; done; "$PYBIN" -c "from pathlib import Path; print(len(Path('正文文件路径').read_text(encoding='utf-8')))"`（**勿直接用 `python3`**：Windows 上它会触发 Microsoft Store 占位程序、exit 49 失败，探测会按 `python3→python→py` 选可用解释器）；macOS/Linux 可用 `wc -m` 备选。完整正文、油管朗读、推文连载、短季每集正文默认必须落在 14,500-16,500 字符；低于 14,500 且用户没有明确批准短稿时，不得报告完成，必须按情节点预算补足。如果字数 < 细纲目标，**对照情节点字数预算定位欠账**：细纲已按预算编排（Σ≥章目标），欠字几乎总是某些密点被写成了带过——找出这些点，一次性重写到各自预算（补具体事例、慢镜头逐拍展开），疏点保持带过，按 plot-core-methods 信息密度高低交替、不均匀注水（低压/关系/信息整理章欠字则补铺垫/互动/表演 beat，不强塞密点——低密点是这类章节定位的设计而非欠账）。一次重写到位，不逐点挤牙膏反复回炉；确属细纲预算不足（Σ<章目标 的规划疏漏）时才回细纲补展开点。旧细纲无 per-point 预算时，先按功能标签派生临时预算（爽点/打脸/反转=密、过场/赶路=疏）再定位欠账。如果字数 > 章目标×1.1，按高侧收敛：压过场、合并疏点、删过渡（镜像细纲高侧规则），不删主线爽点凑数。非短季普通长篇章节如有项目自定短章目标，可按用户/项目目标执行，但 90% 只能作为警戒线，不能作为完成线。重写后重新统计，落进目标区间再进入步骤 9。
...
**场景状态变化门槛**：逐场景写出 `进入状态 → 触发动作/选择 → 离开状态`。离开时至少改变一项：信息、目标、关系、资源、身份/地位、风险、地点控制权或下一步选择；只有换地点、重复情绪、解释旧信息不算变化。连续两个场景没有可验证状态变化时，合并、压缩或重写。每章另核对 `本章承诺 → 兑现/部分兑现 → 代价 → 新问题`，不能只靠章尾悬念掩盖中段停滞。
...
按场景加载，不一次全部加载。完整的阶段索引、主题权威文件和配套文件见 [references/reference-index.md](references/reference-index.md)。
