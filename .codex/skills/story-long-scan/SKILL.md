...
description: "长篇网文实时扫榜与选题验证。采集或读取起点、番茄、晋江、七猫等长篇榜单，记录时间戳、样本质量与平台指标，区分个例/候选信号/可复验趋势，并输出题材机会、失败风险和下一步验证。用于长篇什么火、起点/番茄/晋江排行、长篇选题；短篇排行用 story-short-scan。"
...
要求只回复数字。选 5 后下一轮只索取平台名。平台确定后，如题材仍未明确，再显示：`1. 全榜概览`、`2. 男频`、`3. 女频`、`4. 现实题材`、`5. 自定义题材`；要求只回复数字，选 5 后下一轮才索取关键词。
