---
name: story-review
description: "小说、网文、章节、设定与大纲的多视角对抗式审查。输出带原始行号、证据、严重度和可执行修复的 findings；full/lean 在 reviewer agents 可用时并行，否则自动降级 solo 并使用内置 rubric。用于审查故事稿、章节质量、结构/角色/一致性问题；不是通用代码审查。"
...
置信度与严重度分开：严重但证据不足的问题写 `confidence: low`，不能因为措辞激烈就升级为 S1。综合裁决时按“同一根因 + 同一位置/影响范围”合并重复 findings，保留证据最强的一条，并把其他视角写入补充说明；不要用四个 agent 的重复票数伪造四个独立问题。修复建议采用最小充分修改，明确要改哪一处状态、因果、台词或场景功能，不默认整章重写。
