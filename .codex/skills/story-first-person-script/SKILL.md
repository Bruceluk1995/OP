...
For push work, lock the `1 + 3 + N` chain from `../story/references/push-retention-chain.md` and the compact oral spine from `references/push-quality-gate.md` before prose: abnormal outcome, ordinary cause/choice, immediate pressure or changed status, audience wait, narrator filter, first proof, irreversible conflict, middle mechanism changes, climax debt, and ending proof action. If any field is missing, do not draft around the gap.
