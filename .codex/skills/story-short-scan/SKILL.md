...
description: "短篇网文实时扫榜与情绪风口验证。采集或读取知乎盐言、七猫、黑岩、点众等短篇样本，记录时间戳、传播/完读代理指标和样本质量，区分短期个例与可复验风口，并输出题材、情绪引擎、饱和风险和复扫时间。用于短篇什么火、知乎故事排行、短篇选题；长篇排行用 story-long-scan。"
...
要求只回复数字。选 5 后下一轮只索取平台名。平台确定后，如题材仍未明确，再显示：`1. 全榜概览`、`2. 情感反转`、`3. 悬疑`、`4. 世情现实`、`5. 自定义题材`；要求只回复数字，选 5 后下一轮才索取关键词。
