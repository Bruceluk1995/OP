...
Lock before outlining:

```text
narrative distance and knowledge boundary:
stable protagonist/supporting-character labels:
what the narrator notices and calls absurd, costly, dangerous, or ironic:
...
Extract approved facts and anchors, create a fresh causal/change outline, and draft from it. Do not convert first person by replacing pronouns with names or convert prose by adding line breaks. Use the active genre skill's rewrite-integrity tool when files exist.

## Hard Rules

- No hidden thoughts or off-screen motives stated as certainty unless the narration explicitly owns an omniscient scope and uses it consistently.
- No `name + は` repetition as a substitute for viewpoint stability.
