# Lao Liu Dynamic Market Learning

Use this contract before commercial premise selection, planning, drafting, or claims that a story is market-validated.
...
9. After drafting, define a small-cost test: title/opening A/B, first 30-second retention proxy, sample upload, reader panel, or limited-traffic release. Feed the result into the next decision instead of freezing one success forever.
...
```text
evidence_mode:
sample_scope_and_date:
validated_packaging:
validated_story_functions:
counterexample_or_saturation:
