...
**执行规则：短篇以读者等待和人物选择为发动机，情绪由具体事件与后果产生。**
...
1. **先定读者等待**。用一句话说明读者想看见谁做什么、付什么代价、得到哪种满足；说不清就不动笔。
2. **先做三个故事核再选一个**。核心必须是 `普通困境 -> 主角反常选择 -> 可见结果 -> 新麻烦 -> 有代价的高潮选择 -> 改变后的结尾`。
3. **开头制造问题债务**。具体事件、人的利害和角色反应先出现；背景只在改变当前选择时补充。
4. **场景必须改变状态**。信息、目标、危险、关系、身份、资源、选择或代价至少变化一项；连续流程场直接合并。
5. **一个结论只证明一次**。第二次展示只有在机制、风险、关系或结果改变时才保留。
6. **中段换推进机制**。不要只延迟同一反转；引入对手适应、成功反噬、关系转向、新代价或主角误判。
7. **第一人称是知识与嘴型**。选择第一人称时同时锁定“我知道什么、先看什么、怎样判断、拒绝承认什么”；不是把第三人称换成“我”。
8. **盲读编辑有权退稿**。先找最早失去继续阅读欲望的位置，按故事核/结构/场景/行文顺序返工，再做格式和去 AI 味。

---

### 长度预算

- 完整短篇/朗读稿无明确规格时，默认目标约 **12,000 字符**；用户或平台给出硬长度时按其执行。
- 长度是预算，不是质量门槛。短于目标时只允许增加新的选择、关系变化、代价、对手反应或成功后的麻烦；没有合适新转折就如实报告实际长度。
- 正文落盘后统计并报告字符数，但禁止为了跨过下限补实验、证据、手续、风景、形容词或重复情绪。

### 执行档

- **快速正文（默认）**：用户已给题材/核心时，只做故事核、变化大纲、正文、盲读重写；不强制扫榜、拆文、agent、ledger 或完整项目文件。
- **完整工程**：用户要求可续写项目、对标资产、角色/追踪/包装时再落全部文件。
- **研究变体**：用户要求市场、仿写、多对标或批量方案时才调用扫榜/拆文并生成多个候选；选定前不写正文。
...
### 基本信息
- 标题（暂定）：{}
- 目标字数：{} 字（无明确规格时约 12,000；这是预算，不是最低交付线）
- 目标平台：{}
- 情绪目标：{读者读完的感受}
...
**长度不足时先问故事是否真的缺一轮新选择/代价；若核心已经完整，保留紧凑版本并报告实际长度。**
...
如果某节明显过短，检查它是否缺少选择、后果或场景变化；功能完整则不补写。
禁止跳过未达标的小节。

> 批量验证更高效：一次性输出多节能让 AI 保持叙事连贯性，
> 批后统计比逐节暂停更符合 AI 的文本生成特性。

> **节长判断**：不用行数速算。以“进入状态是否被人物选择改变、下一问题是否成立”判断该节是否完成。
...
**写完后对照 小节大纲.md 检查**：人物想要什么、作了什么选择、状态改变了什么、留下什么问题；缺功能才重写，不按节长补写。
...
- [ ] 完整正文无明确规格时以约 12,000 字符为预算，已用 Python 统计并如实报告实际长度
...
- [ ] 每节以一次完整的欲望-选择-变化为单位，没有为了最低节长补子事件
- [ ] 节数 = 小节大纲规划节数（不得合并/省略）
- [ ] 身体部位同一词全文 ≤ 5 次
- [ ] 「像」≤ 10 处
