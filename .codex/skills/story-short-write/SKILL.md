...
- 完整正文、油管朗读、推文连载、短季分集正文默认目标为 **14,500-16,500 字符**。用户明确指定其他长度时按用户目标执行；用户只说“短篇/完整正文/一集/朗读稿”而未给字数时，一律按 15,000 字符级别处理。
- 不得把旧模板里的 8,000 字下限当成交付目标。8,000 只适用于用户明确要求短稿、试写、样章或平台硬性短篇规格的情况。
- 正文落盘后必须用 Python 字符统计验证。低于 14,500 字符且用户没有明确批准短稿时，禁止报告完成；必须补充有功能的子事件、证据、对话、关系变化、反转铺垫或收束，不得靠景物、形容词、空泛情绪、氛围段落凑字。
...
如果用户只有模糊想法 → 显示情绪数字菜单并要求只回复数字：

| 编号 | 情绪类型 | 适合场景 | 难度 | 市场热度 |
|------|----------|----------|------|----------|
| 1 | 意难平 | 虐恋、遗憾、错过 | 中 | 🔥🔥🔥 |
| 2 | 反转震撼 | 悬疑、身份错位 | 高 | 🔥🔥🔥 |
| 3 | 爽感释放 | 打脸、逆袭 | 低 | 🔥🔥 |
...
### 基本信息
- 标题（暂定）：{}
- 目标字数：{} 字（默认 14,500-16,500；用户明确指定短稿时可低于 14,500）
- 目标平台：{}
- 情绪目标：{读者读完的感受}
...
**⚠️ 字数不足 = 正文未完成。禁止在字数未达标时结束。必须继续展开有功能的剧情内容直到达标。**
