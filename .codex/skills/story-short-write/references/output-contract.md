...
| 6 | 综合判断 | `拆文报告.md`（综合段） + `_meta.json`（写 structure_counts） | 五项门槛 / 爆点机制 / 话题性 / 有证据的共鸣 / 可借鉴结构 / 节奏速报 |
...
| `拆文报告.md` | 分析叙事主体 | 读「故事核」「读者债务」「状态变化」「中段机制」「人物选择」「叙述声线」「五项门槛」「可借鉴结构」段，是 writer 的主输入 |
...
# 通过：按约 12,000 字符预算输出同题材新短篇，长度不作为质量通过条件；prose 有源文对话节奏和画面感
