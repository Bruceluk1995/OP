...
**使用条件**：只有故事确实需要三种不同机制、且每层都改变人物选择与后果时才使用；与字数无关。
...
- 反转应发生在必要线索已经可回看、旧解释仍被读者相信的时候；不要用固定百分比代替铺垫判断。
- 揭示后必须留出人物选择、代价与关系余波；若只能解释真相而不能改变行动，反转来得太晚或功能不足。
- 双反转必须改变两次不同的认知或目标，不能把一个真相拆成两次公布。
