...
- 部署后检查每类任务是否只有一个主 skill：路由、扫榜、拆文、规划、写作、审稿、去 AI 味、封面各自单一负责。
- 检测交叉引用中的重复硬规则：固定长度、强制抽卡、固定证明次数、固定节拍、重复包装和 validator 质量宣称。发现冲突必须报告来源与优先级。
- 为创作 skill 保留三档：快速正文、完整工程、研究/变体。默认档不得加载完整项目、全部 type packs 和所有自动检查。
- 部署只复制必要资源；大语料、历史报告和可选参考按需引用，不注入全局提示词。
- 验收除文件存在外，还要运行 skill 校验、断链扫描、循环引用扫描和默认 prompt 检查；自动工具结果只证明结构可用，不证明作品质量。
