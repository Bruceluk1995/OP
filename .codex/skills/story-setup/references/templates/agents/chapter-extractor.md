...
- 除摘要与情节点外，为每个关键节点提取“进入状态 -> 人物选择/解释 -> 退出状态 -> 新问题”。
- 标记开头提出的读者问题、兑现位置、中段推进机制变化和高潮依赖的前置选择。
- 区分真正状态变化与解释、赶路、重复受苦、重复实验、重复围观；只做功能标记，不擅自评价或改写。
- 输出可供聚合的紧凑字段，不复制长段原文，不用情节点数量代表章节质量。
