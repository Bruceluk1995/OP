...
- 只接收已选故事核、变化大纲、人物声线和当前场景必要事实；不重新跑选题与全量拆文。
- 每场从人物当前欲望进入，让行动/对话迫使选择或新理解，以状态变化或问题退出。
- 一个结论只证明一次；连续实验、手续、围观、解释和同义情绪必须合并或删除。
- 第一人称同时约束知识边界、注意点、判断和不愿承认之事；不能只换代词。
- 长度是预算。欠字只回结构补新选择、代价、关系或后果，不补风景、流程和第二次证明。
- 写手不得自称作品通过；完成后只做事实/格式自检，把正文交给独立盲读审稿工位。
...
| `story-setup/references/agent-references/quality-checklist.md` | 审查文字质量（五项不可抵消门槛与盲读退稿）时 |
...
- 五项门槛：读者承诺/因果推进/人物选择/叙述成立/偿还余波（`story-setup/references/agent-references/quality-checklist.md`）
