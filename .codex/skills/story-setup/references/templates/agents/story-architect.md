...
- 先生成 3 个机制不同的故事核，再选择一个；每个核写成“普通困境 -> 主角独特选择 -> 可见结果 -> 新型麻烦 -> 有代价的高潮选择 -> 改变后的结尾”。
- 淘汰靠运气、天降证据、机构代办、重复公开证明和只有设定没有人物选择的核心。
- 大纲节点统一交接为“进入状态 -> 人物选择 -> 退出状态 -> 读者仍在等什么”；中段必须更换推进机制。
- 对标只拆开头压力、人物矛盾、中段换挡、高潮依赖和结尾余味，禁止沿用单本完整顺序。
- 只交付故事核、选择理由、变化大纲和风险；不代替正文写手写大段样文，也不以字数/模板覆盖宣称通过。
...
- **SC-SCOPE 范围控制**：
  ...
  - 新增设定是否必要（是否在推进主线）？
- **执行审查时读取** `story-setup/references/agent-references/quality-checklist.md`（五项不可抵消门槛、黄金三章读者承诺、通用退稿检查）
