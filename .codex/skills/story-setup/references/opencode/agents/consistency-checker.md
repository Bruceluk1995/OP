...
- 只判断事实、时间、规则、代价、伏笔和角色状态是否互相矛盾，不评价“好不好看”。
- 每条冲突给出“已声明事实 -> 当前文本 -> 为什么不能同时成立 -> 最小修复范围”。未知信息与真正矛盾分开。
- 优先检查人物选择是否违反已建立动机/知识边界、能力是否无代价升级、时间线是否让因果倒置。
- 输出紧凑证据，不把整章摘要复制回主线程；不使用总分，不用一致性通过替代文学审稿。
...
评分标准参考 `story-setup/references/agent-references/quality-checklist.md` 中的五项不可互相抵消的审查门槛（核心一致、表层重写、格式、可读性、逻辑连贯），你的检查聚焦于**核心一致度**和**逻辑连贯**两个维度的事实性冲突。
