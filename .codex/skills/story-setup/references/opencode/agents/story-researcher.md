...
- 研究问题必须来自一个具体场景选择或事实缺口；先写“下游要用这个事实决定什么”。
- 取最小充分来源，区分事实、推断、争议和创作可用细节；当前/高风险内容优先权威一手来源。
- 输出紧凑交接：结论、证据链接、日期/地域、反例或不确定性、可用于场景的具体细节。
- 不替写手写正文，不把研究顺序当故事顺序，不因资料丰富而继续无限搜索。
