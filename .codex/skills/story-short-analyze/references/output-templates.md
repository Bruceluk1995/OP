...
| Stage 6 评估报告 | Stage 6 综合判断 | 五项门槛 + 爆点机制 + 有证据的共鸣 + 可借鉴结构 |
| 同类对比（可选） | 同类对比 | 差异化亮点 |
...
<!-- output to: 拆文报告.md（综合段：五项门槛判断 + 爆点机制 + 话题性 + 共鸣 + 核心手法 + 可借鉴结构 + 同类型写作动作 + 节奏速报）+ _meta.json.structure_counts（只记录事实计数，详见下方「_meta.json.structure_counts 产出模板」） -->
...
| 门槛 | 判断 | 原文证据与机制 |
|------|------|----------------|
| 读者承诺 | {成立/局部失效/失效} | {开头让读者等待什么；最早退出风险} |
| 因果推进 | {成立/局部失效/失效} | {关键选择如何改变后续；有无重复证明} |
| 人物选择 | {成立/局部失效/失效} | {不可逆选择及其代价；是否靠外力解决} |
| 叙述声线 | {成立/局部失效/失效} | {知识边界、观察偏好、判断方式} |
| 偿还与余波 | {成立/局部失效/失效} | {开头债务如何偿还；结局产生什么新状态} |
