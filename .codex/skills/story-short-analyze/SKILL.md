...
要求只回复数字。选 1 后下一轮只索取路径；选 2 后下一轮只索取原文；选 3 后依次索取标题与平台/来源。
...
word_count = 全文字数
  ...
  ├─ 15,000 - 20,000   → 灰区：显示 `1. 按短篇拆`、`2. 改用长篇拆文`，要求只回复数字
  └─ > 20,000          → 提示「此文字数 {N} 偏长，建议改用 /story-long-analyze」，并显示 `1. 改用长篇拆文`、`2. 仍按短篇拆`，要求只回复数字
```
...
存在 _meta.json？
  ├─ 否 → 直接进入新一轮拆解
  └─ 是 → 显示数字菜单并要求只回复数字：
       1. 覆盖：归档旧产出到 拆文库/{书名}/_archive_{时间戳}/ 后从 Stage 2 重跑
       2. 续跑：读 _meta.json.last_stage_in_progress（非空 → 从该 Stage 整段重跑）
