...
description: "短篇网文拆文。面向单篇短故事、番茄短篇、故事会、知乎盐选及追妻/世情/重生/虐渣等通俗短篇，按 Stage 2-6 拆出故事核、情节节点、情感线、反转、人物与写作手法并落盘。用于短篇拆文、分析这个短故事、约 2 万字以下单篇；多章长篇优先 story-long-analyze。"
...
**证据契约**：每个关于故事核、爆点、情绪、反转、人物功能或写法的结论，都必须回指情节节点 ID、功能分段、原文位置或短引用。报告保留“结论 → 证据 → 可复用功能 → 失效条件”；证据不足就标低置信/待补，不拿题材模板替原文作证。

## 短篇功能拆解优先级

- 第一项不是“有哪些反转”，而是读者被许诺看见什么，以及开头用哪个具体事件让下一句变得必要。
- 拆出唯一故事核：日常困境 -> 主角反常选择 -> 第一次结果 -> 结果制造的新麻烦 -> 有代价的高潮选择 -> 改变后的结尾。
- 每个情节节点标记人物选择和状态变化；只解释、受苦、赶路、试验、围观或再次证明同一结论的节点标为可删/可并。
- 建立问题债务表：问题何时提出、读者如何猜、何时更新、在哪里偿还。开头立刻解释完或结尾未偿还都必须标红。
- 单独提取第一人称的知识边界、镜头选择和判断口吻；不能把代词频率当第一人称质量。
- 输出“可复用功能”和“必须换掉的具体元素”，让后续仿写组合多个样本的不同长处，而不是换名复刻一本。
...
| 6 | 综合判断 + `_meta.json` 写事实计数 | 全部数据 | 五项门槛判断 + 爆点机制 + 话题性 + 有证据的共鸣分析 + 可借鉴结构 + 节奏速报 + **按事实写入 `_meta.json.structure_counts`**。 | 每项判断有原文证据；事实计数只描述样本，不参与文学质量判定 |
