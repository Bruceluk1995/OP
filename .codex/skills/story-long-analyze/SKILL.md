...
description: "长篇网文拆文。面向多章连载、长篇小说和黄金三章分析，按 Stage 0-6 产出逐章证据、节奏/情绪模块、设定关系、文风与汇总报告并落盘到 拆文库/{书名}/。用于长篇拆文、分析黄金三章、完整/系统拆解；单篇短故事或约 2 万字以下文本优先 story-short-analyze。"
...
要求只回复数字。选 1 后下一轮只索取文件路径；选 2 后下一轮只索取原文；选 3 后依次索取书名与平台；选 4 后先显示 2-3 本带编号的候选书，再要求只回复数字。

### 统一入口

确认拆解对象后直接进入拆解管道（Phase 2）。**没有快速/深度分叉**——只有一条深度拆解管道，跑到 Stage 1（黄金三章）后自动停靠产出快速预览报告。

**无文本路径时**：重新显示上方来源数字菜单，不用开放式问题索取多项信息。拿到原文后进入管道。
...
3. **显示明确二选一数字菜单**：
   > 「黄金三章已拆完，快速预览报告见 `快速预览.md`。请选择下一步：
   > 1. 继续全量拆解（Stage 2-6：逐章摘要 / 聚合分析 / 设定关系 / 汇总报告 / 文风，预计耗时 {基于章节数粗估}）
   > 2. 就到这里
   > 请只回复数字。」
   - 选 1 → 读 `_progress.md`，从 **Stage 2** 续跑，**不重跑 Stage 0/1**。
   - 选 2 → 管道结束，`_progress.md` 状态保持 `paused_after_stage1`，告知用户「之后可随时 `/story-long-analyze` 同一本书，会自动从 Stage 2 续跑」。
